...
const sampleIssue = `[{"id":1,"type":"open_source","severity":"high","rule_id":"test-rule"}]`
...
func TestRun_FileInput(t *testing.T) {
	...
	if err := os.WriteFile(path, []byte(sampleIssue), 0o600); err != nil {
	...
	if !strings.Contains(out, `"tool": "aikido"`) {
...
func TestRun_ConversionError(t *testing.T) {
	withArgs(t)
	withStdin(t, `[{"id":1,"severity":"high","rule_id":"test-rule"}]`) // missing type

	if _, err := captureStdout(t, run); err == nil {
		t.Fatal("expected a conversion error for an issue with no type, got nil")
	}
}
