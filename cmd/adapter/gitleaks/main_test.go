...
const sampleFinding = `[{"RuleID":"test-rule","File":"test.go"}]`
...
func TestRun_FileInput(t *testing.T) {
	...
	if err := os.WriteFile(path, []byte(sampleFinding), 0o600); err != nil {
	...
	if !strings.Contains(out, `"tool": "gitleaks"`) {
...
func TestRun_ConversionError(t *testing.T) {
	withArgs(t)
	withStdin(t, `[{"File":"test.go"}]`) // missing RuleID

	if _, err := captureStdout(t, run); err == nil {
		t.Fatal("expected a conversion error for a finding with no RuleID, got nil")
	}
}
