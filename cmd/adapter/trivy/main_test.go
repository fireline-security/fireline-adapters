...
const sampleReport = `{"Results":[{"Vulnerabilities":[{"VulnerabilityID":"CVE-1","PkgName":"foo"}]}]}`
...
func TestRun_FileInput(t *testing.T) {
	...
	if !strings.Contains(out, `"tool": "trivy"`) {
...
func TestRun_ConversionError(t *testing.T) {
	withArgs(t)
	withStdin(t, `{"Results":[{"Vulnerabilities":[{"PkgName":"foo"}]}]}`) // missing VulnerabilityID

	if _, err := captureStdout(t, run); err == nil {
		t.Fatal("expected a conversion error for a vulnerability with no VulnerabilityID, got nil")
	}
}
